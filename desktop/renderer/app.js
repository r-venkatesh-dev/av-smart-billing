...
const state = { page: "dashboard", license: null, device: null, business: null, security: null, unlocked: false, query: "", invoiceDraft: { customerMode: "saved", items: [{}] }, posDraft: { items: [], customerId: "", walkInName: "", walkInPhone: "", paymentMethod: "CASH", amountReceived: "", search: "", heldBillId: null }, autoPrintInvoice: false };
...
async function boot() {
  try {
    ...
    state.security = bootstrap.security;
    if (!state.license.active) return renderActivation();
    if (state.security?.enabled && !state.unlocked) return renderAppLock();
    renderShell();
...
function renderAppLock() {
  root.innerHTML = `<main class="activation"><section class="activation-card"><img class="brand-logo brand-logo-large" src="app-logo.png" alt="AV Smartbilling logo"><p class="eyebrow" style="margin-top:18px">Protected billing data</p><h1>Unlock AV Smartbilling</h1><p class="muted">Enter your 6-digit application PIN to continue.</p><form id="unlock-form" class="form-grid" style="margin-top:24px">${field("Application PIN", "pin", "", { type: "password", required: true, full: true })}<div class="field full"><button class="button" type="submit">Unlock software</button></div></form>${state.security?.touchIdAvailable ? `<button id="touch-unlock" class="button secondary" style="width:100%;margin-top:12px">Unlock with Touch ID</button>` : ""}</section></main>`;
  const pin = document.querySelector('[name="pin"]');
  pin.inputMode = "numeric"; pin.maxLength = 6; pin.pattern = "[0-9]{6}"; pin.focus();
  document.getElementById("unlock-form").addEventListener("submit", async (event) => {
    event.preventDefault();
    try {
      const result = await invoke("security:verify", { pin: pin.value });
      if (!result.valid) throw new Error("The PIN is incorrect.");
      unlockApplication();
    } catch (error) { notify(error.message, true); pin.select(); }
  });
  document.getElementById("touch-unlock")?.addEventListener("click", async () => {
    try { await invoke("security:biometric"); unlockApplication(); } catch (error) { notify(error.message, true); }
  });
}
...
function lockApplication() {
  if (!state.security?.enabled || !state.license?.active) return;
...
function resetInactivityTimer() {
  clearTimeout(state.inactivityTimer);
  if (!state.security?.enabled || !state.unlocked) return;
  state.inactivityTimer = setTimeout(lockApplication, (state.security.inactivityMinutes || 5) * 60_000);
}

for (const eventName of ["pointerdown", "keydown", "mousemove"]) document.addEventListener(eventName, resetInactivityTimer, { passive: true });

function renderActivation() {
  const expired = state.license?.activated && !state.license.active;
  root.innerHTML = `<main class="activation"><section class="activation-card"><img class="brand-logo brand-logo-large" src="app-logo.png" alt="AV Smartbilling logo"><p class="eyebrow" style="margin-top:18px">AV Smartbilling Desktop</p><h1>${expired ? "Validate this installation" : "Activate this device"}</h1><p class="muted">Billing data is stored locally in SQLite and works offline. Internet is needed only for activation, periodic validation, cloud backup and restore.</p>${state.license?.error ? `<p class="notice error">${esc(state.license.error)}</p>` : ""}
...
function renderShell() {
  ...
  if (!state.globalFilterBound) {
    document.addEventListener("keydown", focusGlobalFilter);
    state.globalFilterBound = true;
  }
}
...
async function renderPos(content) {
  const [allProducts, allCustomers, heldBills, paymentQr] = await Promise.all([invoke("billing:products"), invoke("billing:customers"), invoke("billing:held-bills"), invoke("payment-qr:get")]);
...
async function renderInvoice(content,id){
  const [invoice,business,paymentQr]=await Promise.all([invoke("billing:invoice",{id}),invoke("billing:settings"),invoke("payment-qr:get")]);
  ...
  const action=`<div class="screen-only invoice-actions"><button class="button secondary" id="back">Back</button><button class="button secondary" id="share-whatsapp">WhatsApp</button><button class="button secondary" id="share-email">Email</button><button class="button secondary" id="save-pdf">Save PDF</button><button class="button secondary" id="print-thermal">Print ${business.thermal_paper_width||80}mm</button><button class="button" id="print-a4">Print A4</button></div>`;
  ...
  document.getElementById("save-pdf").addEventListener("click",async()=>{document.body.dataset.printMode="a4";try{const result=await invoke("document:save-pdf",{fileName:invoice.invoice_number,pageSize:"A4",kind:"invoice"});if(!result.canceled)notify(`Invoice PDF saved to ${result.path}`);}catch(error){notify(error.message,true);}finally{delete document.body.dataset.printMode;}});
  const shareMessage=`Hello ${invoice.customer_name},\n\nYour invoice ${invoice.invoice_number} from ${business.company_name} is ready.\nTotal: ${money(invoice.total_in_paise)}\nPayment status: ${invoice.status}\n\nThank you.`;
  document.getElementById("share-whatsapp").addEventListener("click",async()=>{try{await invoke("external:whatsapp",{phone:invoice.customer_phone,message:shareMessage,invoiceNumber:invoice.invoice_number});}catch(error){notify(error.message,true);}});
  document.getElementById("share-email").addEventListener("click",async()=>{try{await invoke("external:email",{email:invoice.customer_email,subject:`Invoice ${invoice.invoice_number} from ${business.company_name}`,body:shareMessage,invoiceNumber:invoice.invoice_number});}catch(error){notify(error.message,true);}});
...
async function renderReports(content, range = {}) {
  const defaultFrom = new Date(); defaultFrom.setDate(1);
  const from = range.from || defaultFrom.toISOString().slice(0, 10);
  const to = range.to || today();
  const data = await invoke("billing:reports", { from, to });
  const max = Math.max(...data.monthly.map((row) => row.sales), 1);
  const invoiceRows = data.invoices.map((row) => `<tr><td>${date(row.issued_at)}</td><td>${esc(row.invoice_number)}</td><td>${esc(row.customer_name)}</td><td>${status(row.status)}</td><td>${money(row.total_in_paise)}</td><td>${money(row.paid_in_paise)}</td><td>${money(row.total_in_paise-row.paid_in_paise)}</td></tr>`).join("");
  content.innerHTML = `${pageHeader("Local analytics","Reports & exports",`Sales from ${date(from)} to ${date(to)}.`)}<form id="report-range" class="panel report-controls screen-only"><div class="report-filter">${field("From", "from", from, { type: "date", required: true })}${field("To", "to", to, { type: "date", required: true })}<button class="button">Apply</button></div><div class="invoice-actions"><button type="button" class="button secondary" data-export="csv">CSV</button><button type="button" class="button secondary" data-export="xls">Excel</button><button type="button" class="button" id="report-pdf">Save PDF</button></div></form><section class="cards"><article class="card"><small>Total sales</small><strong>${money(data.totals.sales)}</strong></article><article class="card"><small>Collected</small><strong>${money(data.totals.received)}</strong></article><article class="card"><small>Outstanding</small><strong>${money(data.totals.outstanding)}</strong></article><article class="card"><small>Invoices</small><strong>${data.totals.invoices}</strong></article></section><div class="dashboard-grid"><section class="surface"><div class="surface-head"><h2>Monthly sales</h2></div>${data.monthly.length?`<div class="trend-chart">${data.monthly.map(row=>`<div class="trend-row"><span>${esc(row.month)}</span><i><b style="width:${Math.max(2,row.sales/max*100)}%"></b></i><strong>${money(row.sales)}</strong></div>`).join("")}</div>`:`<p class="empty">No sales data in this period.</p>`}</section><section class="surface"><div class="surface-head"><h2>Payment summary</h2></div>${data.paymentMethods.length?`<div class="compact-list">${data.paymentMethods.map(row=>`<div><span><b>${esc(row.method.replaceAll("_"," "))}</b><small>${row.payment_count} payment${row.payment_count===1?"":"s"}</small></span><strong>${money(row.amount_in_paise)}</strong></div>`).join("")}</div>`:`<p class="empty">No payments in this period.</p>`}</section></div><section class="surface report-invoices"><div class="surface-head"><h2>Invoices</h2><span>${data.invoices.length} records</span></div>${data.invoices.length?`<div class="table-wrap"><table><thead><tr><th>Date</th><th>Invoice</th><th>Customer</th><th>Status</th><th>Total</th><th>Collected</th><th>Balance</th></tr></thead><tbody>${invoiceRows}</tbody></table></div>`:`<p class="empty">No invoices in this period.</p>`}</section>`;
  document.getElementById("report-range").addEventListener("submit", (event) => { event.preventDefault(); renderReports(content, Object.fromEntries(new FormData(event.currentTarget))); });
  function csvCell(value) { return `"${String(value ?? "").replaceAll('"', '""')}"`; }
  const exportRows = [["Date","Invoice","Customer","Status","Total (INR)","Collected (INR)","Balance (INR)"], ...data.invoices.map((row) => [row.issued_at.slice(0,10),row.invoice_number,row.customer_name,row.status,(row.total_in_paise/100).toFixed(2),(row.paid_in_paise/100).toFixed(2),((row.total_in_paise-row.paid_in_paise)/100).toFixed(2)])];
  content.querySelectorAll("[data-export]").forEach((button) => button.addEventListener("click", async () => {
    try {
      const format = button.dataset.export;
      const contentValue = format === "csv" ? `\ufeff${exportRows.map((row) => row.map(csvCell).join(",")).join("\r\n")}` : `\ufeff<html><head><meta charset="utf-8"></head><body><table>${exportRows.map((row,index)=>`<tr>${row.map((cell)=>`<${index?"td":"th"}>${esc(cell)}</${index?"td":"th"}>`).join("")}</tr>`).join("")}</table></body></html>`;
      const result = await invoke("file:save-export", { format, content: contentValue, fileName: `sales-report-${from}-to-${to}` });
      if (!result.canceled) notify(`Report saved to ${result.path}`);
...
async function renderSettings(content){const [business,security,paymentQr,activity]=await Promise.all([invoke("billing:settings"),invoke("security:status"),invoke("payment-qr:get"),invoke("activity:list")]);state.security=security;content.innerHTML=`${pageHeader("This computer","Settings","Business identity, security, payment, invoice and encrypted cloud configuration.")}<form id="settings-form" class="panel form-grid">${field("Business name","companyName",business.company_name,{required:true})}${field("Contact person","contactPerson",business.contact_person)}${field("Email","email",business.email,{type:"email"})}${field("Phone","phone",business.phone)}${field("GSTIN","gstin",business.gstin)}${field("State code","stateCode",business.state_code)}${field("Invoice prefix","invoicePrefix",business.invoice_prefix,{required:true})}${field("Low-stock threshold","lowStockThreshold",business.low_stock_threshold,{type:"number",step:"0.001",min:0})}<label class="field"><span>Thermal receipt width</span><select class="select" name="thermalPaperWidth"><option value="80" ${business.thermal_paper_width!==58?"selected":""}>80 mm</option><option value="58" ${business.thermal_paper_width===58?"selected":""}>58 mm</option></select></label>${field("Address","address",business.address,{textarea:true,full:true})}${field("Default invoice terms","invoiceTerms",business.invoice_terms,{textarea:true,full:true})}${field("Invoice / receipt footer","invoiceFooter",business.invoice_footer,{textarea:true,full:true})}<div class="form-actions field full"><button class="button">Save local settings</button></div></form><div class="settings-grid"><section class="surface settings-card"><p class="eyebrow">Payment</p><h2>UPI payment QR</h2><p class="muted">Show your business QR during UPI checkout and on invoices with an outstanding balance.</p>${paymentQr.configured?`<img class="settings-qr" src="${paymentQr.dataUrl}" alt="Configured payment QR">`:"<p class=\"notice\">No payment QR configured.</p>"}<div class="form-actions"><button class="button" id="pick-payment-qr">${paymentQr.configured?"Replace":"Choose"} QR image</button>${paymentQr.configured?`<button class="button secondary" id="remove-payment-qr">Remove</button>`:""}</div></section><section class="surface settings-card"><p class="eyebrow">Security</p><h2>Application lock</h2><p class="muted">Protect local billing records with a 6-digit PIN and automatic inactivity lock.</p>${security.enabled?`<p class="notice">App lock is enabled · ${security.inactivityMinutes}-minute timeout${security.touchIdAvailable?" · Touch ID available":""}.</p><form id="disable-lock-form">${field("Current PIN","pin","",{type:"password",required:true,full:true})}<div class="form-actions"><button type="button" class="button secondary" id="lock-now">Lock now</button><button class="button danger">Disable lock</button></div></form>`:`<form id="enable-lock-form">${field("New 6-digit PIN","pin","",{type:"password",required:true,full:true})}<label class="field full"><span>Lock after inactivity</span><select class="select" name="inactivityMinutes"><option value="1">1 minute</option><option value="5" selected>5 minutes</option><option value="15">15 minutes</option><option value="30">30 minutes</option></select></label><div class="form-actions"><button class="button">Enable application lock</button></div></form>`}</section></div><section class="surface" style="padding:24px"><p class="eyebrow">Optional online migration</p><h2 style="margin-top:7px">Cloud backup and restore</h2><p class="muted">Local SQLite remains the source of truth. Each explicit backup creates a separate encrypted recovery point.</p><p class="warning">Restoring replaces the current local billing database. A complete SQLite safety copy is created automatically before replacement.</p><div id="cloud-status" class="notice">Cloud status has not been checked.</div><div id="cloud-history"></div><div class="cloud-grid"><article class="cloud-card"><h3>Backup to cloud</h3><p class="muted">Upload a new encrypted version without deleting earlier recovery points.</p><button class="button" id="cloud-backup">Backup local data</button></article><article class="cloud-card"><h3>Restore from cloud</h3><p class="muted">Choose a recovery point above, then restore it after activating this computer.</p><button class="button secondary" id="cloud-restore">Restore selected backup</button></article></div><div class="form-actions"><button class="button secondary" id="validate-license">Validate license online</button></div></section><section class="surface"><div class="surface-head"><h2>Security & activity history</h2><span>Latest ${activity.length}</span></div>${activity.length?`<div class="activity-list">${activity.slice(0,30).map(row=>`<div><span><b>${esc(row.action.replaceAll("_"," "))}</b><small>${esc(row.details||"Local desktop action")}</small></span><time>${date(row.createdAt,true)}</time></div>`).join("")}</div>`:`<p class="empty">No desktop activity recorded yet.</p>`}</section>`;
  ...
  document.getElementById("lock-now")?.addEventListener("click",lockApplication);
