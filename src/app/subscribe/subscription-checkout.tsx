...
export function SubscriptionCheckout({
  ...
  async function recordFailure(payload: Record<string, unknown>) {
    await fetch("/api/subscriptions/failure", {
      method: "POST",
  ...
  async function pay() {
    ...
    try {
        ...
        handler: async (response: RazorpayResponse) => {
          ...
          try {
            ...
            if (!verification.ok || !result.licenseKey) {
              setMessage(
                result.message ??
                  "Payment needs manual verification. Please contact support and do not pay again.",
              );
              setBusy(false);
              return;
            }
            ...
            setMessage(
              "Payment was received, but verification was interrupted. Please contact support and do not pay again.",
            );
          } finally {
            setBusy(false);
          }
      ...
      checkout.on("payment.failed", (response) => {
        ...
        void recordFailure({
          subscriptionOrderId: order.subscriptionOrderId,
          razorpayOrderId: error.metadata?.order_id ?? order.razorpayOrderId,
          razorpayPaymentId: error.metadata?.payment_id,
          error,
          cancelled: false,
        });
        setMessage(
          error.description ?? "Payment failed. You can try again safely.",
        );
        setBusy(false);
      });
      checkout.open();
  ...
  async function copyKey() {
    try {
      ...
      setMessage(
        "Copy was blocked by the browser. Select the key and copy it manually.",
      );
    }
  }
      ...
      <section className="mx-auto max-w-2xl border border-emerald-300 bg-white p-5 shadow-sm sm:p-6">
        ...
        <div className="mt-5 border border-amber-300 bg-amber-50 p-4">
          <p className="text-sm font-bold text-amber-900">
            One-time view — copy this key now.
          </p>
          <p className="mt-1 text-xs leading-5 text-amber-800">
            After you close or leave this page, the full key cannot be shown
            again.
          </p>
          ...
          <button
            type="button"
            onClick={copyKey}
            className="mt-4 flex h-11 w-full items-center justify-center gap-2 bg-[#057c73] text-xs font-bold uppercase tracking-[.1em] text-white"
          >
            <Copy size={17} />
        ...
        {message ? (
          <p className="mt-4 bg-amber-50 p-3 text-sm text-amber-900">
            {message}
          </p>
        ) : null}
        <button
          type="button"
          onClick={finish}
          className="mt-4 h-11 w-full border border-[#057c73] text-xs font-bold uppercase tracking-[.1em] text-[#035f58]"
        >
  ...
  return (
    <section className="grid gap-4 lg:grid-cols-[1fr_300px]">
      <div className="border border-[#dfe3e1] bg-white p-4 sm:p-5">
        <div className="mb-5 flex items-center gap-3 text-xs font-bold uppercase tracking-[.1em]">
          <span
            className={`grid size-7 place-items-center rounded-full ${step === "DETAILS" ? "bg-[#057c73] text-white" : "bg-emerald-50 text-emerald-700"}`}
          >
            {step === "DETAILS" ? "1" : <Check size={15} />}
          </span>
          <span>Shop details</span>
          <span className="h-px flex-1 bg-[#dfe3e1]" />
          <span
            className={`grid size-7 place-items-center rounded-full ${step === "PLAN" ? "bg-[#057c73] text-white" : "bg-[#edf0ef] text-[#6d716f]"}`}
          >
            2
          </span>
        ...
        {step === "DETAILS" ? (
          ...
          >
            ...
            <Field
              ...
              value={details.gstin}
              onChange={(value) => update("gstin", value.slice(0, 15))}
            ...
            <label className="sm:col-span-2">
              ...
              <textarea
                ...
                maxLength={500}
                rows={3}
                className="focus-ring w-full border border-[#dfe3e1] bg-white px-3 py-2.5 text-sm"
              />
            </label>
            <button
              type="submit"
          ...
          <div>
            ...
            {message ? (
              <p className="mt-4 bg-amber-50 p-3 text-sm leading-6 text-amber-900">
                {message}
              </p>
            ) : null}
            <div className="mt-4 flex flex-col-reverse gap-3 sm:flex-row sm:justify-between">
              <button
                ...
                disabled={busy}
                className="h-12 border border-[#dfe3e1] px-6 text-xs font-bold uppercase tracking-[.1em]"
              >
                Back
              </button>
              <button
  ...
  return (
    <label>
      ...
      <input
        ...
        className="focus-ring h-11 w-full border border-[#dfe3e1] bg-white px-3 text-sm"
      />
    </label>
  );
}
