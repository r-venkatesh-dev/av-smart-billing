...
export async function createLicense(_state: EntityFormState, formData: FormData): Promise<EntityFormState> {
  ...
  const [customer, plan] = await Promise.all([
    ...
    supabase.from("plans").select("id, name, max_devices, validation_window_days, allow_online_billing, allow_cloud_backup, allow_reports_exports, status").eq("id", parsed.data.planId).single(),
  ]);
