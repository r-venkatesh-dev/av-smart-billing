...
export default async function LicenseDetailPage({ params }: PageProps<"/admin/licenses/[id]">) {
  ...
  return <div className="space-y-7">
    ...
    {admin.role === "OWNER" || admin.role === "ADMIN" ? <section className="surface flex items-center justify-between gap-4 p-5"><div><h2 className="font-bold">Full activation key</h2><p className="mt-1 text-xs text-[#667085]">Password verification is required every time. Closing or reloading hides the key.</p></div><LicenseKeyReveal licenseId={license.id} recoverable={license.recoverableKey} /></section> : null}
