...
export function BillingProductForm({ product }: { product?: { id: string; name: string; sku: string; description: string; unit: string; priceInPaise: number; taxRateBasisPoints: number; stockQuantity: number; status: string } }) {
  ...
  return <><form ref={formRef} action={action} onSubmit={submit} className="surface space-y-5 p-6"><div className="grid gap-5 sm:grid-cols-2"><label><span className="mb-2 block text-sm font-semibold">Product name</span><input name="name" required defaultValue={product?.name ?? ""} className={inputClass} /></label><label><span className="mb-2 block text-sm font-semibold">SKU</span><input name="sku" required defaultValue={product?.sku ?? ""} className={inputClass} /></label><label><span className="mb-2 block text-sm font-semibold">Price (₹)</span><input name="priceInRupees" type="number" min="0" step="0.01" required defaultValue={product ? (product.priceInPaise / 100).toFixed(2) : ""} className={inputClass} /></label><label><span className="mb-2 block text-sm font-semibold">Tax rate (%)</span><input name="taxRatePercent" type="number" min="0" max="100" step="0.01" defaultValue={product ? (product.taxRateBasisPoints / 100).toFixed(2) : "0"} required className={inputClass} /></label><label><span className="mb-2 block text-sm font-semibold">{product ? "Current stock" : "Opening stock"}</span><input name="stockQuantity" type="number" min="0" step="0.001" defaultValue={product?.stockQuantity ?? 0} required className={inputClass} /></label><label><span className="mb-2 block text-sm font-semibold">Unit</span><input name="unit" defaultValue={product?.unit ?? "unit"} required className={inputClass} /></label><ThemedSelect name="status" label="Status" options={statusOptions} defaultValue={product?.status ?? "ACTIVE"} required /><label className="sm:col-span-2"><span className="mb-2 block text-sm font-semibold">Description</span><textarea name="description" defaultValue={product?.description ?? ""} rows={3} className="focus-ring w-full rounded-xl border border-[#dfe3eb] bg-white p-3 text-sm" /></label></div><Message state={state} /><Actions cancel="/billing/products" label={product ? "Save product" : "Create product"} /></form>{product && confirming ? <div className="fixed inset-0 z-[80] grid place-items-center bg-[#26272a]/55 p-5" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setConfirming(false); }}><div role="dialog" aria-modal="true" aria-labelledby={`save-product-${product.id}`} className="w-full max-w-md border border-[#dfe3e1] bg-[#f7f8f7] p-6 text-left shadow-2xl"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-bold uppercase tracking-[.16em] text-[#057c73]">Confirm product update</p><h2 id={`save-product-${product.id}`} className="mt-2 text-2xl font-normal">Save changes to {product.name}?</h2></div><button type="button" onClick={() => setConfirming(false)} aria-label="Close confirmation" className="focus-ring grid size-9 place-items-center border border-[#dfe3e1]"><X size={17} /></button></div><p className="mt-4 text-sm leading-6 text-[#6d716f]">This will update the product details, price, tax, stock and availability in Supabase. Existing invoice line-item history will not be changed.</p><div className="mt-6 flex justify-end gap-3 border-t border-[#dfe3e1] pt-5"><button type="button" onClick={() => setConfirming(false)} className="border border-[#dfe3e1] px-4 py-2.5 text-[10px] font-bold uppercase tracking-[.1em]">Review again</button><button type="button" onClick={confirmSave} className="bg-[#057c73] px-4 py-2.5 text-[10px] font-bold uppercase tracking-[.1em] text-white">Confirm &amp; save</button></div></div></div> : null}</>;
}
