...
const inputClass =
  "focus-ring h-11 w-full rounded-xl border border-[#dfe3eb] bg-white px-3 text-sm";
...
function SubmitButton({ label }: { label: string }) {
  ...
  return (
    <button
      disabled={pending}
      className="focus-ring rounded-xl bg-[#5b4df5] px-5 py-3 text-[11px] font-bold uppercase tracking-[.1em] text-white disabled:opacity-60"
    >
...
function FieldError({ state, name }: { state: EntityFormState; name: string }) {
    ...
    <form action={formAction} className="surface space-y-5 p-6">
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <label>
          ...
          <input
            ...
            className={inputClass}
          />
          <FieldError state={state} name="contactPerson" />
        </label>
        <label>
        ...
        <label>
          <span className="mb-2 block text-sm font-semibold">GSTIN</span>
          <input
            name="gstin"
            defaultValue={customer?.gstin ?? ""}
            className={inputClass}
          />
          <FieldError state={state} name="gstin" />
        </label>
        <label>
      ...
      <div className="flex justify-end gap-3 border-t border-[#eaecf0] pt-5">
        <Link
          ...
          className="rounded-xl border border-[#dfe3eb] px-5 py-3 text-[11px] font-bold uppercase tracking-[.1em]"
        >
          Cancel
        </Link>
...
export function PlanForm({
  ...
  return (
    <form action={formAction} className="surface space-y-5 p-6">
      <div className="grid gap-5 sm:grid-cols-2">
        <label>
          <span className="mb-2 block text-sm font-semibold">Plan name</span>
          <input
            name="name"
            required
        ...
        <label>
          ...
          <input
            ...
            max="100"
            required
            defaultValue={plan?.maxDevices ?? 1}
            className={inputClass}
          />
        </label>
        <label>
          ...
          <input
            ...
            max="365"
            required
            defaultValue={plan?.validationWindowDays ?? 30}
            className={inputClass}
          />
        </label>
        <label>
      ...
      <div className="flex justify-end gap-3 border-t border-[#eaecf0] pt-5">
        ...
        <SubmitButton label={plan ? "Save plan" : "Create plan"} />
      </div>
    </form>
  );
}
...
export function PlatformSettingsForm({
  ...
  settings: {
    defaultValidationWindowDays: number;
    expiryWarningDays: number;
    licenseIssuer: string;
  };
}) {
  const [state, action] = useActionState(updatePlatformSettings, initialState);
  return (
    <form action={action} className="surface space-y-6 p-6">
      <div>
        <h2 className="font-bold">Default license policy</h2>
        <p className="mt-1 text-sm text-[#667085]">
          New plans and license workflows can use these defaults.
        </p>
      </div>
      <div className="grid gap-5 sm:grid-cols-2">
        <label>
          ...
          <input
            name="defaultValidationWindowDays"
            type="number"
            min="1"
            max="365"
            defaultValue={settings.defaultValidationWindowDays}
            className={inputClass}
          />
        </label>
        <label>
        ...
        <label className="sm:col-span-2">
          <span className="mb-2 block text-sm font-semibold">
            Activation payload issuer
          </span>
          <input
            name="licenseIssuer"
            type="url"
            required
            defaultValue={settings.licenseIssuer}
            className={inputClass}
          />
          <FieldError state={state} name="licenseIssuer" />
        </label>
      </div>
      {state.message ? (
        <p className="rounded-xl bg-rose-50 p-3 text-sm text-rose-700">
...
export function LicenseForm({
  ...
  const [state, action] = useActionState(createLicense, initialState);
  ...
  const planOptions = plans.map((plan) => ({
    value: plan.id,
    label: plan.name,
    description: `${plan.maxDevices} device${plan.maxDevices === 1 ? "" : "s"} · ${plan.validationWindowDays}-day offline window`,
  }));
  return (
    <div className="space-y-5">
      <form action={action} className="surface space-y-5 p-6">
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <div>
            <ThemedSelect
              ...
              label="Plan"
              options={planOptions}
              placeholder="Select license plan"
              required
            />
            <FieldError state={state} name="planId" />
          </div>
          <ThemedDatePicker
            name="expiresAt"
        ...
        <p className="border-l-2 border-[#057c73] bg-[#e6f2f0] p-3 text-xs leading-5 text-[#035f58]">
          Device limit and offline validation window are copied from the
          selected plan. The plaintext key is revealed only once.
        </p>
        {state.message && !state.licenseKey ? (
          <p className="bg-rose-50 p-3 text-sm text-rose-700">
            {state.message}
          </p>
        ) : null}
        <div className="flex justify-end gap-3 border-t border-[#eaecf0] pt-5">
          <Link
            href="/admin/licenses"
            className="border border-[#dfe3eb] px-5 py-3 text-[11px] font-bold uppercase tracking-[.1em]"
          >
            Cancel
          </Link>
          <SubmitButton label="Generate license" />
        </div>
      </form>
      {state.licenseKey ? (
        <section className="border border-amber-300 bg-amber-50 p-6">
          <p className="text-[10px] font-bold uppercase tracking-[.16em] text-amber-800">
            Copy now — shown only once
          </p>
          <code className="mt-3 block break-all font-mono text-2xl font-bold tracking-[.08em] text-[#26272a]">
            {state.licenseKey}
          </code>
          <p className="mt-3 text-sm leading-6 text-amber-900">
            {state.message} Send this key securely to the customer.
          </p>
          <div className="mt-5 flex flex-wrap gap-3">
            <button
              ...
              className="bg-[#057c73] px-4 py-2.5 text-[10px] font-bold uppercase tracking-[.1em] text-white"
            >
              Copy license key
            </button>
            <Link
              href="/activate"
              target="_blank"
              className="border border-[#057c73] px-4 py-2.5 text-[10px] font-bold uppercase tracking-[.1em] text-[#035f58]"
            >
              Test activation
            </Link>
            {state.licenseId ? (
              <Link
                href={`/admin/licenses/${state.licenseId}`}
                className="border border-amber-400 px-4 py-2.5 text-[10px] font-bold uppercase tracking-[.1em]"
              >
                Open license
              </Link>
            ) : null}
